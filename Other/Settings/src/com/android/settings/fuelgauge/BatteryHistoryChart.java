...
public class BatteryHistoryChart extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mLargeMode) {
			//2014-9-26 added by Jacob to fix bug#7104:Battery - history information recording interface font and recording information icon overlay
            if (mHavePhoneSignal) {
                canvas.drawText(mPhoneSignalLabel, textStartX,
                        height - mPhoneSignalOffset - mTextDescent - 6, mTextPaint);
            }
            if (mHaveGps) {
                canvas.drawText(mGpsOnLabel, textStartX,
                        height - mGpsOnOffset - mTextDescent - 6, mTextPaint);
            }
            if (mHaveWifi) {
                canvas.drawText(mWifiRunningLabel, textStartX,
                        height - mWifiRunningOffset - mTextDescent - 6, mTextPaint);
            }
            canvas.drawText(mWakeLockLabel, textStartX,
                    height - mWakeLockOffset - mTextDescent - 6, mTextPaint);
            canvas.drawText(mChargingLabel, textStartX,
                    height - mChargingOffset - mTextDescent - 6, mTextPaint);
            canvas.drawText(mScreenOnLabel, textStartX,
                    height - mScreenOnOffset - mTextDescent - 6, mTextPaint);
