...
public class FileDBOP {
    ...
    private static final String SD = Environment.getExternalStorageDirectory()
            .getAbsolutePath();
	//2014-10-27 add by qiuxintai to fix bug#7528:can not set music in sd card as alarm ringtone
	private static final String SD1 = "/storage/sdcard1";
    ...
    public static boolean startMediaScan(final Context context, final String path) {
        ...
        intent.setData(Uri.fromFile(new File(SD)));
        context.sendBroadcast(intent);
        Log.d(TAG, "send whole scan intent ............path = " + path);
        return true;
    }
    ...
    public static Uri updateMediaAudioDB(Context context, String path) {
        ...
        path = path.replaceAll("'", "''");
        Uri uri;
        //add by gezhao to fix bug#3754: can not set up mp3 file in SD card and U disk as ringstones 
        if (path.startsWith("/storage/sdcard1")||path.startsWith(SD) || path.startsWith("/storage/usbcard1")) {
            uri = MediaStore.Audio.Media.EXTERNAL_CONTENT_URI;
        } else {
