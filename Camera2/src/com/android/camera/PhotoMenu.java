...
public class PhotoMenu extends PieController
    ...
    public void initialize(PreferenceGroup group) {
		...
		//2014-12-19 tangjingliang add image storage selector ******************* start
		item = makeItem(R.drawable.ic_storage_select);
		final ListPreference storagePre = group.findPreference(CameraSettings.KEY_STORAGE_SELECT);
        ...
        item.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(PieItem item) {
                ListPrefSettingPopup popup = (ListPrefSettingPopup) mActivity.getLayoutInflater().inflate(
                        R.layout.list_pref_setting_popup, null, false);
                if(Storage.isExistsSDcard()){
                	storagePre.setEntries(mActivity.getResources().getStringArray(R.array.pref_camera_storage_entries));
                	storagePre.setEntryValues(mActivity.getResources().getStringArray(R.array.pref_camera_storage_entryvalues));       	
                }else{
                	storagePre.setEntries(mActivity.getResources().getStringArray(R.array.pref_camera_storage_entries_nosdcard));
                	storagePre.setEntryValues(mActivity.getResources().getStringArray(R.array.pref_camera_storage_entryvalues_nosdcard));
                	Storage.updateImageDirectory(mActivity.getResources().getString(R.string.camera_internal_path));
                }
                storagePre.setValue(Storage.DIRECTORY);
                popup.initialize(storagePre);
                popup.setSettingChangedListener(PhotoMenu.this);
                mUI.dismissPopup();
                mPopup = popup;
                mUI.showPopup(mPopup);
        ...
        more.addItem(item);
		//2014-12-19 tangjingliang add image storage selector ******************* end
    ...
    public void onSettingChanged(ListPreference pref) {
        ...
        if (notSame(pref, CameraSettings.KEY_CAMERA_HDR, mSettingOff)) {
            ...
            //tangjingliang
        } else if(notSame(pref, CameraSettings.KEY_STORAGE_SELECT, Parameters.SCENE_MODE_AUTO)){
			Storage.updateImageDirectory(pref.getValue());
		}
